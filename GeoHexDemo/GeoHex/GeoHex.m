...
+(XY*) loc2xy:(double)lat longitude:(double)lon {
	...
	y *= H_BASE / 180.0;
    return [[XY alloc] initWithX:x y:y];
}
...
+(NSString*) encode:(double)lat longitude:(double)lon level:(int)level {
    return [GeoHex getZoneByLocation:lat longitude:lon level:level].code;
}
...
+(Zone*) getZoneByLocation:(double)lat longitude:(double)lon level:(int)level {
    ...
    return [[Zone alloc] initWitLat:z_loc.lat
                                lon:z_loc.lon
                                  x:h_x
                                  y:h_y
                              level:([h_code_r length] - 2)
                               code:h_code_r];
}
